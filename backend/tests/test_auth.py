...
async def test_register_duplicate_email(async_client: AsyncClient):
    ...
    await async_client.post(
        "/auth/register", json={"email": "dupe@example.com", "password": TEST_PASSWORD}
    )

    # Register again with the same email
    response = await async_client.post(
        "/auth/register", json={"email": "dupe@example.com", "password": "NewPassword123!"}
    )
...
async def test_resend_verification_unverified_success(async_client: AsyncClient, db: AsyncSession):
    ...
    await async_client.post(
        "/auth/register", json={"email": "resend_unverified@example.com", "password": TEST_PASSWORD}
    )

    # 2. Resend verification
    response = await async_client.post(
        "/auth/resend-verification", json={"email": "resend_unverified@example.com"}
    )
    assert response.status_code == 202
    assert response.json()["message"] == "Verification email sent"
    ...
    result = await db.execute(
        select(User).where(User.email_lower == "resend_unverified@example.com")
    )
    user = result.scalar_one()
...
async def test_resend_verification_verified_silent_success(
    async_client: AsyncClient, db: AsyncSession
):
    # 1. Register and verify user
    await async_client.post(
        "/auth/register", json={"email": "resend_verified@example.com", "password": TEST_PASSWORD}
    )
    ...
    # 2. Resend verification
    response = await async_client.post(
        "/auth/resend-verification", json={"email": "resend_verified@example.com"}
    )
...
async def test_resend_verification_nonexistent_user_silent_success(
    async_client: AsyncClient, db: AsyncSession
):
    # Resend for nonexistent email
    response = await async_client.post(
        "/auth/resend-verification", json={"email": "nonexistent_resend@example.com"}
    )
...
async def test_verify_email_success(async_client: AsyncClient, db: AsyncSession):
    ...
    await async_client.post(
        "/auth/register", json={"email": "verify_me@example.com", "password": TEST_PASSWORD}
    )
    ...
    response = await async_client.get(
        f"/auth/verify-email?token={raw_token}", follow_redirects=False
    )
...
async def test_login_unverified_fails(async_client: AsyncClient):
    await async_client.post(
        "/auth/register", json={"email": "unverified@example.com", "password": TEST_PASSWORD}
    )

    response = await async_client.post(
        "/auth/login", json={"email": "unverified@example.com", "password": TEST_PASSWORD}
    )
...
async def test_login_success_issues_tokens(async_client: AsyncClient, db: AsyncSession):
    await async_client.post(
        "/auth/register", json={"email": "login@example.com", "password": TEST_PASSWORD}
    )
    await make_user_verified(db, "login@example.com")

    response = await async_client.post(
        "/auth/login", json={"email": "login@example.com", "password": TEST_PASSWORD}
    )
...
async def test_login_invalid_password(async_client: AsyncClient, db: AsyncSession):
    await async_client.post(
        "/auth/register", json={"email": "wrongpass@example.com", "password": TEST_PASSWORD}
    )
    await make_user_verified(db, "wrongpass@example.com")

    response = await async_client.post(
        "/auth/login", json={"email": "wrongpass@example.com", "password": "WrongPassword1!"}
    )
...
async def test_login_lockout_after_10_attempts(async_client: AsyncClient, db: AsyncSession):
    await async_client.post(
        "/auth/register", json={"email": "lockout@example.com", "password": TEST_PASSWORD}
    )
    ...
    for _ in range(10):
        resp = await async_client.post(
            "/auth/login", json={"email": "lockout@example.com", "password": "WrongPassword1!"}
        )
    ...
    resp = await async_client.post(
        "/auth/login", json={"email": "lockout@example.com", "password": TEST_PASSWORD}
    )
...
async def test_refresh_token_rotation_and_grace_period(async_client: AsyncClient, db: AsyncSession):
    # 1. Login
    await async_client.post(
        "/auth/register", json={"email": "refresh@example.com", "password": TEST_PASSWORD}
    )
    await make_user_verified(db, "refresh@example.com")

    login_resp = await async_client.post(
        "/auth/login", json={"email": "refresh@example.com", "password": TEST_PASSWORD}
    )
...
async def test_logout_clears_session(async_client: AsyncClient, db: AsyncSession):
    # 1. Login
    await async_client.post(
        "/auth/register", json={"email": "logout@example.com", "password": TEST_PASSWORD}
    )
    await make_user_verified(db, "logout@example.com")

    await async_client.post(
        "/auth/login", json={"email": "logout@example.com", "password": TEST_PASSWORD}
    )
