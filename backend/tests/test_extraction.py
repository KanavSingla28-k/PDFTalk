...
def test_extract_txt_returns_text():
    with patch(
        "app.services.extraction.s3_client.download_file_streaming",
        return_value=MockStreamingBody(b"Hello plain text"),
    ):
...
def test_extract_txt_replacement_char_on_bad_bytes():
    ...
    with patch(
        "app.services.extraction.s3_client.download_file_streaming",
        return_value=MockStreamingBody(bad_bytes),
    ):
        result = "".join(extract_text("user/doc/file.txt", "text/plain"))
...
def test_extract_image_only_pdf_triggers_ocr():
    ...
    with (
        patch("app.services.extraction.s3_client.download_file", return_value=pdf_bytes),
