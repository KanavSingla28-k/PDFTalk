...
os.environ["DATABASE_URL_SYNC"] = "postgresql+psycopg://pdftalk:3wYd8pmybim4JXe8vmmNQf37zzpY0Oll@localhost:5433/pdftalk"    # pragma: allowlist secret
os.environ["DATABASE_URL"] = "postgresql+asyncpg://pdftalk:3wYd8pmybim4JXe8vmmNQf37zzpY0Oll@localhost:5433/pdftalk"
...
async def upload_document(client: httpx.AsyncClient, pdf_bytes: bytes, index: int, token: str) -> tuple[str, float]:
...
async def poll_document(client: httpx.AsyncClient, doc_id: str, start_time: float, token: str) -> float:
    if not doc_id:
        return -1.0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--size", type=int, default=10, help="Approximate size factor for dummy PDF")
