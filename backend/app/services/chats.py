...
async def list_chats(user_id: uuid.UUID, limit: int, offset: int, db: AsyncSession) -> tuple[list[Chat], int]:
...
async def get_chat_with_messages(chat_id: uuid.UUID, user_id: uuid.UUID, db: AsyncSession) -> tuple[Chat, list[str]]:
...
async def rename_chat(chat_id: uuid.UUID, user_id: uuid.UUID, new_title: str, db: AsyncSession) -> Chat:
