...
export function Citation({ filename, documentId }: CitationProps) {
  ...
  const handleClick = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      alert('Could not open the document. It may have been deleted or expired.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <>
      ...
      {isMarkdown && isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setIsModalOpen(false)}>
          <div 
            className="w-full max-w-4xl max-h-[85vh] bg-white rounded-xl shadow-xl flex flex-col overflow-hidden"
            onClick={(e) => e.stopPropagation()}
            role="dialog"
            aria-modal="true"
            aria-labelledby="modal-title"
          >
            <div className="px-6 py-4 border-b border-[var(--gray-200)] flex-shrink-0 flex items-center justify-between bg-white">
